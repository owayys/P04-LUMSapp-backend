    ...
    return new Promise((resolve, reject) => {
        ...
        pdf(file)
            .then(function (data) {
                ...
                let [stu_credits, stu_cgpa] = data.text
                    .split("CREDITS")[3]
                    .split("\n")[2]
                    .split("TAKEN TOWARDS GPA")[1]
                    .split("CGPA");
                let admitted_year = data.text
                    .split("Year Admitted:")[1]
                    .split("\n")[1]
                    .trim()
                    .split("-")[0];
                let semesters = data.text
                    .split("CREDITS")[2]
                    .split(/\n\s*\n/)
                    .slice(1);
                let sem_info = [];
                ...
                !/^(200[1-9]|20[1-9]\d)$/.test(admitted_year)
                    ? (error_flag = true)
                    : null;
                ...
                for (let semester of semesters) {
                    ...
                    for (let course of courses) {
                        ...
                        let course_credits = course.substring(
                            lastDigitIndex,
                            lastDigitIndex + 1
                        );
                        let courseCodeMatch = tempString.match(
                            /([A-Z]{2,4}\d{2,4})\s*(.*)/
                        );
                        ...
                        let secondSpaceIndex = tempString.indexOf(
                            " ",
                            tempString.indexOf(" ") + 1
                        ); // Splitting Course Code and Name (e.g., 'CS 100 Course_Name' -> ['CS 100', 'Course_Name'])

                        let course_code = tempString.substring(
                            0,
                            secondSpaceIndex
                        );
                        let course_name = tempString.substring(
                            secondSpaceIndex + 1
                        );

                        if (course_grade.includes("R")) {
                            // Removing R from Replaced Course Grade
                            course_grade = course_grade.split("R")[0];
                        }

                        if (
                            course_code != "" &&
                            !course_grade.includes("#") &&
                            !course_grade.includes("W")
                        ) {
                            ...
                            // Pattern matching for validation
                            !/^[A-Z]{2,4} \d{3}[A-Z]?$/.test(course_code)
                                ? (error_flag = true)
                                : null;
                            !/^[0-9]$/.test(course_credits)
                                ? (error_flag = true)
                                : null;
                            !/^[A-Z]?[+-]?$/.test(course_grade)
                                ? (error_flag = true)
                                : null;
                        }
                    }
